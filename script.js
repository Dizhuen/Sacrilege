...
let idleTimer;
let descInterval; // Global interval var to clear it properly
...
async function setupAudio() {
    try {
        ...
        proceduralCtx = new AudioContext(); // UI Beeps
        audioCtx = new AudioContext(); // Music
...
class PsychosisSystem {
    constructor() {
        ...
        this.isActive = false;
        this.phrases = [
            "ARE YOU SURE?", "NO RETURN", "CONSUME", "VOID", "THEY ARE WATCHING", 
            "WAKE UP", "ERROR", "NOTHING IS REAL", "BUY BUY BUY", "SYSTEM FAILURE",
            "DO NOT LOOK", "RUN", "010101", "NULL"
        ];
        this.timers = [];
    }
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        // Re-attach button listener here, but also ensure it's attached via onclick attribute
        ...
        setTimeout(() => {
            ...
            // This ensures cards are visible even if observer fails initially
            const forceReveal = () => {
                document.querySelectorAll('.reveal-item').forEach(el => el.classList.add('active'));
            };
            
            initScrollReveal();
            forceReveal(); // Call once immediately
            
            // Call again shortly after to catch stragglers
...
function renderSections(collections) {
    ...
    if(collections.length === 0 && productsData.length > 0) {
        ...
        collections.forEach(col => {
            ...
            // Added 'active' class to section header to force visibility
        ...
        if(orphans.length > 0) {
            ...
            // Added 'active' class to ARCHIVE header to force visibility
        ...
        // Immediate force reveal for all rendered items to ensure they are visible under the preloader
...
function getProductCardHTML(p) {
    ...
    // Added 'active' class here to ensure immediate visibility upon rendering
    return `
    <div class="reveal-item active group relative spotlight-card rounded-md hover-trigger pointer-events-auto" style="z-index: 1;">
        <div class="absolute top-2 left-2 z-10 bg-black/50 backdrop-blur px-2 py-1 border border-white/10 pointer-events-none">
            <span class="font-mono text-[10px] ${isTotallySoldOut ? 'text-neutral-500' : 'text-red-500'}">${isTotallySoldOut ? 'SOLD_OUT' : p.tag || 'NEW'}</span>
        </div>
        <div class="relative aspect-[4/5] overflow-hidden bg-neutral-900 ${isTotallySoldOut ? 'grayscale opacity-50' : 'grayscale group-hover:grayscale-0'} transition-all duration-700 ease-out cursor-pointer pointer-events-auto" onclick="openProductModal(${p.id})">
            <img src="${p.image}" loading="lazy" class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110 pointer-events-none" />
            <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                    <div class="bg-red-600/90 text-black font-wide font-bold px-4 py-2 text-xs tracking-widest backdrop-blur-sm">VIEW ARTIFACT</div>
            </div>
        </div>
        <div class="mt-4 flex flex-col items-start gap-1 p-2 pointer-events-none">
            <h3 class="font-serif text-lg leading-none text-theme-color group-hover:text-red-500 transition-colors">${p.name}</h3>
            <div class="w-full flex justify-between items-center border-t border-theme-light pt-2 mt-1"><span class="font-mono text-[10px] text-theme-muted">${p.category || '///'}</span><span class="font-mono text-sm text-theme-muted">$${p.price}</span></div>
        </div>
    </div>`;
}
...
function generateSigils() {
    ...
    const createShape = () => {
        ...
        const stroke = '#DC2626';
        if(type === 'circle') {
            svg = `<svg width="100%" height="100%" viewBox="0 0 100 100"><circle cx="50" cy="50" r="48" fill="none" stroke="${stroke}" stroke-width="0.5" stroke-dasharray="5,5" /></svg>`;
        } else if(type === 'line') {
            svg = `<svg width="100%" height="100%" viewBox="0 0 100 100"><line x1="0" y1="50" x2="100" y2="50" stroke="${stroke}" stroke-width="1" /></svg>`;
        } else {
            svg = `<svg width="100%" height="100%" viewBox="0 0 100 100"><polygon points="50,5 95,95 5,95" fill="none" stroke="${stroke}" stroke-width="0.5" /></svg>`;
        }
...
window.openProductModal = function(productId) {
    ...
    generateSigils(); // New Sigil logic
    ...
    btn.disabled = true; 
    btn.classList.add('grayscale', 'cursor-not-allowed', 'opacity-50', 'bg-neutral-800'); 
    btn.classList.remove('bg-red-600');
    
    // Explicitly attach the click handler to the global function
    ...
    descInterval = setInterval(() => {
        ...
        const remaining = desc.length - charIndex;
        const randomCharsCount = Math.min(remaining, 20);
        
        for(let i = 0; i < randomCharsCount; i++) {
            displayText += String.fromCharCode(65 + Math.floor(Math.random() * 26));
        }
        
        descEl.innerText = displayText;
        charIndex += speed;
    }, 50);

    slideImages = [product.image, ...(product.gallery || [])];
...
// Global addToCart function for robustness
...
window.selectSize = function(size, btnElement) {
    ...
    document.querySelectorAll('.size-btn').forEach(btn => {
        if(!btn.classList.contains('disabled')) {
            btn.style.backgroundColor = ''; btn.style.color = '';
            btn.classList.remove('bg-theme-color', 'text-theme-main', 'border-theme-color'); 
            btn.classList.add('border-theme');
        }
    });
    btnElement.classList.remove('border-theme');
    btnElement.style.backgroundColor = 'var(--text-color)'; 
    btnElement.style.color = 'var(--bg-color)'; 
    btnElement.style.borderColor = 'var(--text-color)';
    
    const addBtn = document.getElementById('confirm-add-btn');
    if(addBtn) {
        addBtn.disabled = false;
        addBtn.classList.remove('grayscale', 'cursor-not-allowed', 'opacity-50', 'bg-neutral-800');
    ...
    list.innerHTML = reviews.map(r => `
        <div class="review-card p-3 mb-2">
            <div class="flex justify-between items-baseline mb-2">
                <span class="font-bold text-sm text-theme-color">${r.name}</span>
                <span class="text-[10px] font-mono text-theme-muted">${new Date(r.created_at).toLocaleDateString()}</span>
            </div>
            ${r.products ? `<div class="text-[9px] text-theme-muted mb-2">RE: ${r.products.name}</div>` : ''}
            ${r.content_satisfied ? `<div class="mb-2 text-xs md:text-sm font-mono text-green-500 bg-green-900/10 p-2 border-l-2 border-green-800"><span class="opacity-50 text-[10px] block mb-1">/// SATISFIED:</span>${r.content_satisfied}</div>` : ''}
            ${r.content_disgruntled ? `<div class="mb-1 text-xs md:text-sm font-mono text-red-500 bg-red-900/10 p-2 border-l-2 border-red-800"><span class="opacity-50 text-[10px] block mb-1">/// DISGRUNTLED:</span>${r.content_disgruntled}</div>` : ''}
            ${!r.content_satisfied && !r.content_disgruntled ? `<p class="text-xs md:text-sm font-mono text-theme-muted leading-relaxed">${r.content}</p>` : ''}
        </div>`).join('');
}
...
function updateCartUI() {
    ...
    container.innerHTML = cart.map((item, index) => `
        <div class="flex gap-4 bg-theme-secondary p-2 border border-theme relative group animate-pulse-fast">
            <div class="relative w-16 h-20"><img src="${item.image}" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all"><div class="absolute bottom-0 right-0 bg-white text-black text-[10px] font-bold px-1">${item.size}</div></div>
            <div class="flex flex-col justify-between w-full"><div><div class="font-serif text-sm text-theme-color">${item.name}</div><div class="font-mono text-xs text-red-500">$${item.price}</div></div><button onclick="removeFromCart(${index})" class="text-[10px] font-mono text-theme-muted text-left hover:text-theme-color hover-trigger">REMOVE</button></div>
        </div>`).join('');
}
...
function initMagneticButtons() {
    ...
    document.querySelectorAll('.magnet-btn').forEach(btn => {
        ...
        btn.addEventListener('mousemove', (e) => {
            ...
            xTo((e.clientX - (rect.left + rect.width / 2)) * 0.4); yTo((e.clientY - (rect.top + rect.height / 2)) * 0.4);
        });
...
function initHoverTriggers() {
    ...
    if(logo) addSound(logo);
}

function initScrollReveal() {
    // UPDATED: Increased margin for smoother loading on mobile and lower threshold
    // NEW: rootMargin increased to 200px to load elements much earlier
    ...
    }, { threshold: 0.05, rootMargin: "200px" }); // Forgiving margin
    document.querySelectorAll('.reveal-item').forEach(el => observer.observe(el));
    
    // SAFETY FALLBACK: Force reveal everything for ALL devices if animation fails
    // This fixes "empty" spaces if observer misfires
    setTimeout(() => {
        document.querySelectorAll('.reveal-item:not(.active)').forEach(el => el.classList.add('active'));
    }, 500);
}
